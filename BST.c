...
void main()
{
	...
	int search;
	int i;
	init_tree(); // 1. создать путсое дерево

	// 1. считать 4 элементов и занести в дерево
	for (i = 0; i < 4; i++)
	{
		scanf("%d", &input_value); // 1.считать
		insert_BST(input_value);  // 1.занести
	}
	printf("\n");
	print(root); // 2. вывод дерева
	printf("\n");
	
	//3. считать 3 элемента и занести в дерево
	for (i = 0; i < 3; i++)
	{
		scanf("%d", &input_value); //3. считать
		insert_BST(input_value); // 3. занести
	}
	printf("\n");
	print(root);// 4. вывод дерева
	printf("\n");
	
	scanf("%d", &search);
	find(search); // 5. поиск предка и потомка

	scanf("%d", &search);
	find(search); // 6. поиск предка и потомка
	
	//7.удалить элемент
	scanf("%d", &delete_value);
	delete_BST(delete_value);
	printf("\n");
	print(root); //8. вывод дерева
	printf("\n");
	
	//9. выполнить левый поворот 
	printf("\n");
	...
	printf("\n");
	//10. показ дерева
	//11. выполнить правый поворот 12. вывести дерево
	printf("\n");
	...
	//13. вывод количество элементов в дереве
	printf("count: %d\n", count_node(root));
	printf("\n");

	//U2F2ZWwnZXYgRGVuaXM=
	//14 очистить дерево 15. вывод дерево
	clear(root);
	print(root);
	printf("\n");
}
